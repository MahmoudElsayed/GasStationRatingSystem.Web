...
function roundingTwoDecimals(value, useFloor) {
    if (useFloor === void 0) { useFloor = false; }
    return (useFloor ? Math.floor : Math.round)(value * 100) / 100;
}
exports.roundingTwoDecimals = roundingTwoDecimals;
function unitsToPixel(val, measureUnit, zoom) {
    ...
    result = result * (zoom);
    return roundingTwoDecimals(result, true);
}
...
function pixelToUnits(val, measureUnit, zoom) {
    ...
    result = result / (zoom);
    return roundingTwoDecimals(result);
}
